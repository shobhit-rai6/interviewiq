...
function Step2Interview({ interviewData, onFinish }) {
  const { interviewId, questions, userName, aiGender: preferredGender = "female" } = interviewData;
  ...
  const [voiceGender, setVoiceGender] = useState(preferredGender);
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      const preferred = isMale ? maleVoice : femaleVoice;
      const fallback = isMale ? femaleVoice : maleVoice;
      const chosen = preferred || fallback || voices[0];

      setSelectedVoice(chosen);
      setVoiceGender(
        chosen?.name?.toLowerCase().includes("david") ||
        chosen?.name?.toLowerCase().includes("mark") ||
        chosen?.name?.toLowerCase().includes("male")
          ? "male"
          : "female"
      );
    };
